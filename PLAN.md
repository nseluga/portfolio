...
- [ ] **C1.2 NBA Shot-Value → reframe as calibration-first EV study** (`audits/audit-nba-shot-value.md`)
  ...
  - Credit the collaboration and link the formal PDF writeup in the repo.
...
- [ ] **C3.3 Contact & next-action:** put email + GitHub + LinkedIn visibly on the home page
      (not just footer), and add a resume link (PDF in `public/`, kept current — needs the
      PDF from Nate; placeholder task until provided). Every page should have an obvious
      next action.
