...
section: "engineering"
date: 2026-07-09
summary: "A personal operating system built on top of Claude Code: 48 custom skills, a structured project-tracking system covering 8 active projects, and a persistent memory layer with 10 fact files. The system compounds — each skill added makes the next session more capable — and is the infrastructure behind all AI-assisted work on this site."
...
The repo is public and openable at https://github.com/nseluga/os. The numbers here — 48 skills, 8 projects, 10 memory files — are counts from the live repo, not projections.
...
**`career-advisor`** (`~/os/skills/career-advisor/skill.md`) — a senior tech-industry hiring advisor persona, applied to portfolio writeups, project pages, and site copy. Without it, I'm writing portfolio content against my own judgment about what reads well to a recruiter; with it, I get a consistent rubric — accuracy first, then clarity, then credibility, then recruiter impact — applied every time I touch site copy. This writeup was reviewed against it. The patio.mdx writeup was reviewed against it. The homepage bio was updated because of it. It compresses what I know about portfolio-writing quality into a thing I can invoke rather than re-derive.

**`dev-team-auto`** (`~/os/skills/dev-team-auto/skill.md`) — an autonomous multi-agent convergence loop: Engineer builds a task, QA gates it, an Optimization Reviewer reviews, a Bug Fixer applies findings, repeating until the item passes or hits a cap. I run this unattended overnight on the portfolio (the v3 content-voice loop running right now is an example). The skill is what makes that possible — it encodes the loop structure, the handoff protocol between agents, and the stopping conditions. Without the skill, running a multi-agent loop requires orchestrating it manually at the start of each session.
...
Eight active projects are tracked under `~/os/projects/`, each with a `README.md` containing frontmatter fields (`status`, `next_step`, `last_active`, `repo`, `priority`) and a body description. This is what a project dashboard I built separately reads to generate a weekly digest — but its primary value is simpler: when I start a session on a project, the context file is already there. I don't re-explain what the project is, what phase it's in, or what the obvious next step is.

### Hooks: automated behaviors at session boundaries

Claude Code supports hooks that fire on events like session start, tool use, and session stop. I use these to auto-load project context, enforce conventions, and run post-session updates. The hooks are configured in `~/.claude/settings.json`, with the templates in `~/os`.
...
What I haven't done yet: formal benchmarking of session quality before and after the system, or A/B comparison of output quality with and without skills on matched tasks. Those would make the "this system helps" claim much stronger. Right now, the evidence is behavioral (I use it daily; it changes what I do) not measured.
